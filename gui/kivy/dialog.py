...
class NFCTransactionDialog(AnimatedPopup):
    ...
    def __init__(self, **kwargs):
        ...
        if NFCSCanner is None:
            from electrum_xvg_gui.kivy.nfc_scanner import NFCScanner
...
class WalletAddressesDialog(CarouselDialog):
    ...
    def __init__(self, **kwargs):
        ...
        if not ScreenAddress:
            from electrum_xvg_gui.kivy.screens import ScreenAddress
        slide = ScreenAddress()
...
class NewContactDialog(Popup):
    ...
    def load_qr_scanner(self):
        global QRScanner
        if not QRScanner:
            from electrum_xvg_gui.kivy.qr_scanner import QRScanner
        qrscr = self.qrscr
        if not qrscr:
    ...
    def on_symbols(self, instance, value):
        ...
        global decode_uri
        if not decode_uri:
            from electrum_xvg_gui.kivy.qr_scanner import decode_uri
...
class ChangePasswordDialog(CreateAccountDialog):
    ...
    mode = OptionProperty('new', options=('new', 'confirm', 'create', 'restore'))
